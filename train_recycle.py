...
import batchgen.dataset_recycle as dataset_rec
...
# tune convolutions to fixed input size
...
class Dataset(torch.utils.data.Dataset):
  ...
  def __getitem__(self, index): #each example consists of patterson + template map and partial structure inputs, and output electron density 
        ...
        # load model prediction template map
        X1 = torch.load('predictions/' + ID + '.pt')
        X1 = X1[0,0,:,:,:]
...
#checkpoint = torch.load('state_recycle.pth')
